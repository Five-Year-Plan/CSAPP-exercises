...
0000000000400da0 <main>:
  ...
  400e32:	e8 67 06 00 00       	callq  40149e <read_line> # 调用读取行的函数
  400e37:	48 89 c7             	mov    %rax,%rdi          # 传给phase_1一个参数，肯定是我们输入的字符串
  400e3a:	e8 a1 00 00 00       	callq  400ee0 <phase_1>
  400e3f:	e8 80 07 00 00       	callq  4015c4 <phase_defused>
  400e44:	bf a8 23 40 00       	mov    $0x4023a8,%edi
  400e49:	e8 c2 fc ff ff       	callq  400b10 <puts@plt>
  400e4e:	e8 4b 06 00 00       	callq  40149e <read_line> # 类似输入
...
0000000000400ee0 <phase_1>: # "Border relations with Canada have never been better."
  400ee0:	48 83 ec 08          	sub    $0x8,%rsp        # 分配8个字节的空间
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi   # 传参数(在main中使用了%rdi保存输入)，肯定是答案
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal>
  400eee:	85 c0                	test   %eax,%eax        # 调用函数后把判断放在%eax，为0表示正确
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>
  400ef2:	e8 43 05 00 00       	callq  40143a <explode_bomb>
  400ef7:	48 83 c4 08          	add    $0x8,%rsp        # 归还空间
  400efb:	c3                   	retq   

0000000000400efc <phase_2>: # "1 2 4 8 16 32" 不应该看read_six_number，比较复杂，观察phase_1就可以猜出答案
  400efc:	55                   	push   %rbp                       
  400efd:	53                   	push   %rbx                       # 这两个需要使用但是没有看到，six函数内
  400efe:	48 83 ec 28          	sub    $0x28,%rsp
  400f02:	48 89 e6             	mov    %rsp,%rsi                  # rsi保存栈指针
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>  # 读了六个数字？
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)                # 如果 M[rsp] == 1
  400f0e:	74 20                	je     400f30 <phase_2+0x34>      #   跳到L2，初始化
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>      #
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>      #
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax            # M[rbx - 4] * 2 -> eax    L1
  400f1a:	01 c0                	add    %eax,%eax                  # 
  400f1c:	39 03                	cmp    %eax,(%rbx)                # 如果 eax != M[rbx]
  400f1e:	74 05                	je     400f25 <phase_2+0x29>      #   就爆炸
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>      #
  400f25:	48 83 c3 04          	add    $0x4,%rbx                  # rbx + 4 -> rbx
  400f29:	48 39 eb             	cmp    %rbp,%rbx                  # 如果 rbp != rbx 
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>      #   跳到L1
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>      # 否则安全
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx             # rsp + 4 -> rbx           L2
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp            # rsp + 18 -> rbp
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b>      # 跳到L1
  400f3c:	48 83 c4 28          	add    $0x28,%rsp
  400f40:	5b                   	pop    %rbx
  400f41:	5d                   	pop    %rbp
  400f42:	c3                   	retq   

0000000000400f43 <phase_3>: # "0 207" "2 707" "3 256" ... 有八组答案均可通过 ...
  400f43:	48 83 ec 18          	sub    $0x18,%rsp
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx               # rsp + 12 -> rcx
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx               # rsp + 8 -> rdx
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi               # "%d %d" -> esi 上面输入六个int，这里好像是两个的意思
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax                    # 0 -> eax
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax                    # if eax > 1
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>        # then 避免爆炸，跳过下一行
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>        # (输入两个整数，rax=2)
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)               # if M[rsp + 8] > 7
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>        # then 爆炸()
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax               # M[rsp + 8]-> eax
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)           # rax含有判断分支的信息，似乎参照是一个long数组
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax                   # 应该是一个switch语句，分情况修改eax
  ...
  400fab:	eb 11                	jmp    400fbe <phase_3+0x7b>        # 一堆全都跳到
  400fad:	e8 88 04 00 00       	callq  40143a <explode_bomb>
  400fb2:	b8 00 00 00 00       	mov    $0x0,%eax                    # default
  400fb7:	eb 05                	jmp    400fbe <phase_3+0x7b>
  400fb9:	b8 37 01 00 00       	mov    $0x137,%eax                  # 怎么肥四？
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax               # 大家都往这里跳，if eax == M[rsp + 12]
  400fc2:	74 05                	je     400fc9 <phase_3+0x86>        # then 避免爆炸，跳过下一行
  400fc4:	e8 71 04 00 00       	callq  40143a <explode_bomb>
  400fc9:	48 83 c4 18          	add    $0x18,%rsp
  400fcd:	c3                   	retq   

0000000000400fce <func4>:                                   # 期待这个函数把eax修改为0，这时程序正确
  400fce:	48 83 ec 08          	sub    $0x8,%rsp
  400fd2:	89 d0                	mov    %edx,%eax            # a = c
  400fd4:	29 f0                	sub    %esi,%eax            # a = a - s
  400fd6:	89 c1                	mov    %eax,%ecx            # 
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx           # 
  400fdb:	01 c8                	add    %ecx,%eax            # a = a + a >> 31     奇+1偶+0
  400fdd:	d1 f8                	sar    %eax                 # a >>= 1             只右边移一位
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx   # c = a + s
  400fe2:	39 f9                	cmp    %edi,%ecx            # if c <= 输入1
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>  # then     ------------------>
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx      # else func4()
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>       # -----------------------------
  400fee:	01 c0                	add    %eax,%eax            # a *= 2
  400ff0:	eb 15                	jmp    401007 <func4+0x39>  # 直接结束
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax            # a = 0    <------------------/
  400ff7:	39 f9                	cmp    %edi,%ecx            # if c >= 输入1 
  400ff9:	7d 0c                	jge    401007 <func4+0x39>  # then 顺利结束，所以当c=输入1时就可以了，算出来是7
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi       # else s = c + 1
  400ffe:	e8 cb ff ff ff       	callq  400fce <func4>       # func4()
  401003:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax # a = 2 * a + 1
  401007:	48 83 c4 08          	add    $0x8,%rsp
  40100b:	c3                   	retq   

000000000040100c <phase_4>: # "7 0" 第一个分析func4，第二个直接看phase_4就行了，暴露很明显
  40100c:	48 83 ec 18          	sub    $0x18,%rsp
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx         # 0 -> rcx
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx         # 0 -> rdx
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi         # addr -> esi 输入地址
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax              # 0 -> eax
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax              # if eax ！= 2 应该又是几个字符串，这回是2个
  40102c:	75 07                	jne    401035 <phase_4+0x29>  # then BOOM!!!
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)         # elif M[rsp + 8] <= 14 发现这是输入的第一个数 
  401033:	76 05                	jbe    40103a <phase_4+0x2e>  # then SAFE... for a while
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx              # 14 -> edx
  40103f:	be 00 00 00 00       	mov    $0x0,%esi              # 0 -> esi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi         # 输入的第一个数 -> edi
  401048:	e8 81 ff ff ff       	callq  400fce <func4>         # 应该是判断两个输入的函数
  40104d:	85 c0                	test   %eax,%eax              # if eax != 0
  40104f:	75 07                	jne    401058 <phase_4+0x4c>  # then BOOM!!! 应该是检查判断结果
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)         # elif M[rsp + 12] == 0 第二个数必然是0
  401056:	74 05                	je     40105d <phase_4+0x51>  # then SAFE!!!
...
000000000040131b <string_length>: # 应该是判断长度的函数
  ...
  401323:	48 83 c2 01          	add    $0x1,%rdx # %rdi + 1 -> %rdx
  401327:	89 d0                	mov    %edx,%eax                        
  401329:	29 f8                	sub    %edi,%eax # %edx - %edi -> %eax  
...
0000000000401338 <strings_not_equal>: # 判断字符串相等的函数发现！有%rdi保存输入地址，%rsi保存答案地址
  ...
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> # 直接返回
  ...
  401396:	ba 01 00 00 00       	mov    $0x1,%edx # 1表示错误的分支
  40139b:	89 d0                	mov    %edx,%eax # %eax保存了判断结果
  40139d:	5b                   	pop    %rbx
  40139e:	5d                   	pop    %rbp
...
000000000040145c <read_six_numbers>:  # phase_2和6的奇妙过程,%rbx和%rbp在这里# 如果(%rsp) = 1会防止爆炸
  40145c:	48 83 ec 18          	sub    $0x18,%rsp                      
  401460:	48 89 f2             	mov    %rsi,%rdx                       # rsi保存当前栈指针+18byte的位置，
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx                  # rsi+4 -> rcx
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax                 # (覆盖了输入)
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)                  # rsi+14 -> M[rsp+8]
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax                 #
  401474:	48 89 04 24          	mov    %rax,(%rsp)                     # M[rsp+10] -> M[rsp]
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9                   # rsi+12 -> r9
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8                   # rsi+8 -> r8
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi                  # 把我的输入改成了这个蜜汁数字
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>    # 发生了什么？
  40148f:	83 f8 05             	cmp    $0x5,%eax                       # 如果%eax > 5可以躲过去 
...
000000000040149e <read_line>:
  ...
  4015bc:	48 89 f0             	mov    %rsi,%rax  # 读取到的输入存入%rax
